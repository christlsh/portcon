...
from PortconOptimizers.optimizers import solve_opt_longshort_dynamic
...
#     factor_bounds = {k:v for k,v in factor_bounds.items() if k in risks_factors}
    ...
    try:
        ...
        weights = solve_opt_longshort_dynamic(last_weights_, ret_1.values.astype('float64'), mandate.trading_costs, cov_adj, srisks.set_index('code').loc[cids], factor_exposure.astype('float64'), mandate, risks_factors, control_factors,turnover_limit,target_vol, min_single_stock_weight)
        if last_weights is None:
        ...
        try:
            weights = solve_opt_longshort_dynamic(last_weights_, ret_1.values.astype('float64'), mandate.trading_costs, cov_adj, srisks.set_index('code').loc[cids], factor_exposure.astype('float64'), mandate, risks_factors, control_factors,turnover_limit*2,target_vol, min_single_stock_weight)
            if last_weights is None:
                weights['last_target_volume'] = 0
                weights['initial_weights'] = 0.0
            else:
            ...
            try:
                weights = solve_opt_longshort_dynamic(last_weights_, ret_1.values.astype('float64'), mandate.trading_costs, cov_adj, srisks.set_index('code').loc[cids], factor_exposure.astype('float64'), mandate, risks_factors, control_factors,turnover_limit*4,target_vol, min_single_stock_weight)
                if last_weights is None:
                    weights['last_target_volume'] = 0
                    weights['initial_weights'] = 0.0
                else:
                ...
                weights = solve_opt_longshort_dynamic(last_weights_, ret_1.values.astype('float64'), mandate.trading_costs, cov_adj, srisks.set_index('code').loc[cids], factor_exposure.astype('float64'), mandate, risks_factors, control_factors,turnover_limit*10,target_vol, min_single_stock_weight)
                if last_weights is None:
                    weights['last_target_volume'] = 0
                    weights['initial_weights'] = 0.0
                else:
