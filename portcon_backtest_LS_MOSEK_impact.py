...
from PortconOptimizers.optimizers import solve_opt_longshort_dynamic_impact
...
parser.add_argument('-impact_ratio_path',help='impact ratio path', default='/data/crypto/live/portcon/impact/')
...
def load_impact(tdt, itvl, impact_ratio_path,AUM, profile):
    impact_ratio = pd.read_parquet(os.path.join(impact_ratio_path,str(tdt)+'.parquet'))
    impact_ratio = impact_ratio[impact_ratio['time']==itvl].copy()
    impact_para = profile['portfolio_constraints']['impact_para'] 
    impact_ratio['impact_ratio'] = impact_ratio['impact_ratio']* (AUM**1.5) * impact_para
    return impact_ratio[['code','impact_ratio']].set_index('code')
...
def run_optimization(tpt,alpha,ptb,profile,last_weights,factor_exposure,srisks,cov_adj,impact):
    time = tpt.time()
    total_cap = AUM
...
#     factor_bounds = {k:v for k,v in factor_bounds.items() if k in risks_factors}
    ...
    mandate.market_impact_tc = impact.loc[cids].values
    factor_exposure['pred'] = ret_1

    try:
        weights = solve_opt_longshort_dynamic_impact(last_weights_, ret_1.values.astype('float64'), mandate.market_impact_tc, cov_adj, srisks.set_index('code').loc[cids], factor_exposure.astype('float64'), mandate, risks_factors, control_factors,turnover_limit,target_vol, min_single_stock_weight)
        if last_weights is None:
        ...
        try:
            weights = solve_opt_longshort_dynamic_impact(last_weights_, ret_1.values.astype('float64'), mandate.market_impact_tc, cov_adj, srisks.set_index('code').loc[cids], factor_exposure.astype('float64'), mandate, risks_factors, control_factors,2*turnover_limit,target_vol, min_single_stock_weight)
            if last_weights is None:
                weights['last_target_volume'] = 0
                weights['initial_weights'] = 0.0
            else:                
            ...
            try:
                weights = solve_opt_longshort_dynamic_impact(last_weights_, ret_1.values.astype('float64'), mandate.market_impact_tc, cov_adj, srisks.set_index('code').loc[cids], factor_exposure.astype('float64'), mandate, risks_factors, control_factors,4*turnover_limit,target_vol, min_single_stock_weight)
                if last_weights is None:
                    weights['last_target_volume'] = 0
                    weights['initial_weights'] = 0.0
                else:
                ...
                weights = solve_opt_longshort_dynamic_impact(last_weights_, ret_1.values.astype('float64'), mandate.market_impact_tc, cov_adj, srisks.set_index('code').loc[cids], factor_exposure.astype('float64'), mandate, risks_factors, control_factors,10*turnover_limit,target_vol, min_single_stock_weight)
                if last_weights is None:
                    weights['last_target_volume'] = 0
                    weights['initial_weights'] = 0.0
                else:
...
if __name__ == "__main__":
    ...
    impact_ratio_path = args.impact_ratio_path
    ...
    for tpt in trading_points:
        ...
        impact = load_impact(tdt,itvl, impact_ratio_path, AUM,profile)
        ...
        run_optimization(tpt,alpha,ptb,profile,last_weights,factor_exposure,srisks,cov_adj,impact)
        print(str(dt.datetime.now()))
        last_tpt = tpt